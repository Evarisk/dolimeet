...
/**
 * \file    core/modules/doliopi/doliopidocuments/trainingsessiondocument/doc_completioncertificatedocument_odt.modules.php
 * \ingroup doliopi
...
class doc_completioncertificatedocument_odt extends SaturneDocumentModel
{
    ...
    public function write_file(SaturneDocuments $objectDocument, Translate $outputLangs, string $srcTemplatePath, int $hideDetails = 0, int $hideDesc = 0, int $hideRef = 0, array $moreParam): int
    {
        ...
        if (getDolGlobalInt('DOLIOPI_SESSION_TRAINER_RESPONSIBLE') > 0) {
            $signatory = $signatory->fetchSignatory('UserSignature', $conf->global->DOLIOPI_SESSION_TRAINER_RESPONSIBLE, 'user');
        } else {
        ...
        if(is_array($signatory) && !empty($signatory)) {
            ...
            if (dol_strlen($signatory->signature) > 0 && $signatory->signature != $langs->transnoentities('FileGenerated')) {
                if ($moreParam['specimen'] == 0 || ($moreParam['specimen'] == 1 && $conf->global->DOLIOPI_SHOW_SIGNATURE_SPECIMEN == 1)) {
                    $tempDir      = $conf->doliopi->multidir_output[$object->entity ?? 1] . '/temp/';
