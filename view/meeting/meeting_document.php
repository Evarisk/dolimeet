...
/**
 *  \file       meeting_document.php
 *  \ingroup    meeting
 *  \brief      Tab for documents linked to Meeting
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->dolimeet->multidir_output[$conf->entity ?: 1]."/meeting/".get_exdir(0, 0, 0, 1, $object);
}
...
$title = $langs->trans("Meeting").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/meeting/meeting_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	$modulepart = 'dolimeet';
	//todo:perms
	//$permissiontoadd = $user->rights->meeting->dolimeet->write;
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->meeting->dolimeet->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;

	//$relativepathwithnofile='dolimeet/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'meeting/'.dol_sanitizeFileName($object->ref).'/';
