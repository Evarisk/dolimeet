...
$hookmanager->initHooks(array('auditsignature', 'globalcard')); // Note that conf->hooks_modules contains array

//Security check
$permissiontoread   = $user->rights->dolimeet->audit->read;
$permissiontoadd    = $user->rights->dolimeet->audit->write;
$permissiontodelete = $user->rights->dolimeet->audit->delete;
...
if (empty($backtopage) || ($cancel && empty($id))) {
	if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
		$backtopage = dol_buildpath('/dolimeet/view/audit/audit_attendants.php', 1) . '?id=' . ($object->id > 0 ? $object->id : '__ID__');
	}
}
...
if ($action == 'addSocietyAttendant') {
	...
	if ( ! $error) {
		$result = $signatory->setSignatory($object->id, 'audit', 'user', array($attendant_id), 'AUDIT_SOCIETY_ATTENDANT', 1);
		if ($result > 0) {
			$usertmp = $user;
...
if ($action == 'addExternalAttendant') {
	...
	if ( ! $error) {
		$result = $signatory->setSignatory($object->id, 'audit', 'socpeople', array($extintervenant_id), 'AUDIT_EXTERNAL_ATTENDANT', 1);
		if ($result > 0) {
...
$title    = $langs->trans("AuditAttendants");
$help_url = '';
...
print dol_get_fiche_head($head, 'attendants', $langs->trans("Audit"), -1, "dolimeet@dolimeet");
...
<div class="wpeo-notice notice-warning">
	<div class="notice-content">
		...
		<div class="notice-subtitle"><?php echo $langs->trans("AuditMustBeValidatedToSign") ?></div>
	</div>
	<a class="butAction" style="width = 100%;margin-right:0" href="<?php echo DOL_URL_ROOT ?>/custom/dolimeet/view/audit/audit_card.php?id=<?php echo $id ?>"><?php echo $langs->trans("GoToValidate") ?></a>;
</div>
<?php endif; ?>
<div class="noticeSignatureSuccess wpeo-notice notice-success hidden">
	<div class="all-notice-content">
		...
		<?php
		if ($signatory->checkSignatoriesSignatures($object->id, 'audit')) {
			print '<a class="butAction" style="width = 100%;margin-right:0" href="' . DOL_URL_ROOT . '/custom/dolimeet/view/audit/audit_card.php?id=' . $id . '">' . $langs->trans("GoToLock") . '</a>';
		}
		?>
	</div>
</div>
...
if ((empty($action) || ($action != 'create' && $action != 'edit'))) {
	...
	$society_intervenants = $signatory->fetchSignatory('AUDIT_SOCIETY_ATTENDANT', $object->id, 'audit');
...
//	}
	...
	$ext_society_intervenants = $signatory->fetchSignatory('AUDIT_EXTERNAL_ATTENDANT', $object->id, 'audit');
