...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->dolimeet->multidir_output[$conf->entity ?: 1]."/audit/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($object->id) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/audit/audit_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	$modulepart = 'dolimeet';
	//todo:perms
	//$permissiontoadd = $user->rights->audit->dolimeet->write;
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->audit->dolimeet->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;
