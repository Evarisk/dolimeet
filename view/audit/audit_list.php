...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'auditlist'; // To manage different context of search
...
$diroutputmassaction = $conf->audit->dir_output.'/temp/massgeneration/'.$user->id;
...
if (empty($reshook)) {
	...
	$objectclass = 'Audit';
	$objectlabel = 'Audit';
	$uploaddir = $conf->dolimeet->audit->dir_output;
...
if (dol_strlen($fromtype) > 0 && !in_array($fromtype, $linkedObjectsArray)) {
	$sql .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'element_element as e on (e.fk_source = ' .$fromid. ' AND e.sourcetype="' . $fromtype . '" AND e.targettype = "dolimeet_audit")';
}
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/audit/audit_card.php', 1).'?id='.$id);
	exit;
}
