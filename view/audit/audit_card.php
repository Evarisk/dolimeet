...
$refAuditMod = new $conf->global->DOLIMEET_AUDIT_ADDON();
...
$upload_dir = $conf->dolimeet->multidir_output[$conf->entity ?: $conf->entity]."/audit/".get_exdir(0, 0, 0, 1, $object);
// Load object
include DOL_DOCUMENT_ROOT.'/core/actions_fetchobject.inc.php'; // Must be include, not include_once.

$permissiontoread = $user->rights->dolimeet->audit->read;
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/dolimeet/view/audit/audit_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/dolimeet/view/audit/audit_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
			}
		}
	}
	...
	if ($action == 'add' && $permissiontoadd) {
		...
		// Initialize object
		$now                   = dol_now();
		$object->ref           = $refAuditMod->getNextValue($object);
	...
	if ($action == 'confirm_delete' && GETPOST("confirm") == "yes")
	{
		...
		$urltogo = DOL_URL_ROOT . '/custom/dolimeet/view/audit/audit_list.php';
		header("Location: " . $urltogo);
		exit;
	}
...
//	include DOL_DOCUMENT_ROOT.'/core/actions_builddoc.inc.php';
		...
		if (is_numeric(GETPOST('model', 'alpha'))) {
			...
			if ($result <= 0) {
				...
				if ($conf->global->DOLIMEET_SHOW_DOCUMENTS_ON_PUBLIC_INTERFACE) {
					...
					$ecmfile->fetch(0, '', 'dolimeet/audit/'.$object->ref.'/'.$filename, '', '', 'dolimeet_audit', $id);
	...
	$triggersendname = 'DOLIMEET_AUDIT_SENTBYMAIL';
...
$title_create = $langs->trans("NewAudit");
$title_edit   = $langs->trans("ModifyAudit");
$help_url     = '';
...
if ($action == 'create') {
	...
	print '<input hidden class="flat" type="text" size="36" name="ref" id="ref" value="'.$refAuditMod->getNextValue($object).'">';
	print $refAuditMod->getNextValue($object);
	print '</td></tr>';
...
//	print '</td></tr>';
	...
	if (!empty($conf->categorie->enabled)) {
		...
		$cate_arbo = $form->select_all_categories('audit', '', 'parent', 64, 0, 1);
...
if (($id || $ref) && $action == 'edit' ||$action == 'confirm_setInProgress') {
	...
	print load_fiche_titre($langs->trans("EditAudit"), '', 'object_'.$object->picto);
...
//	print '</td></tr>';
	...
	if ($conf->categorie->enabled) {
		...
		$cate_arbo = $form->select_all_categories('audit', '', 'parent', 64, 0, 1);
		$c = new Categorie($db);
		$cats = $c->containing($object->id, 'audit');
		$arrayselected = array();
		if (is_array($cats)) {
			foreach ($cats as $cat) {
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'confirm_setInProgress' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Audit"), -1, "dolimeet@dolimeet");
	...
	if (($action == 'clone' && (empty($conf->use_javascript_ajax) || ! empty($conf->dol_use_jmobile)))		// Output when action = clone if jmobile or no js
		...
		$formquestionclone = array(
			'text' => $langs->trans("CloneAudit", $object->ref),
		);

		$formconfirm .= $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('ToClone'), $langs->trans('ConfirmCloneAudit', $object->ref), 'confirm_clone', $formquestionclone, 'yes', 'actionButtonClone', 350, 600);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/dolimeet/view/audit/audit_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	// Project
...
//			print '<a class="'. ($object->status == 0 ? 'butAction" id="actionButtonSign" href="' . DOL_URL_ROOT . '/custom/dolimeet/audit_signature.php'.'?id='.$object->id.'&mode=init&token='.newToken().'"' : 'butActionRefused classfortooltip" title="'. $langs->trans('AlreadySigned').'"')  .' >' . $langs->trans("Sign") . '</a>' . "\n";
//			print '<span class="' . ($object->status == 1 ? 'butAction"  id="actionButtonLock"' : 'butActionRefused classfortooltip" title="' . dol_escape_htmltag($langs->trans("AuditMustBeSigned")) . '"') . '>' . $langs->trans("Lock") . '</span>';
