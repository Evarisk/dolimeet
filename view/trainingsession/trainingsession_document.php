...
/**
 *  \file       trainingsession_document.php
 *  \ingroup    trainingsession
 *  \brief      Tab for documents linked to TrainingSession
 */
...
$langs->loadLangs(array("dolimeet@dolimeet", "companies", "other", "mails"));
...
$hookmanager->initHooks(array('dolimeetdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->dolimeet->multidir_output[$conf->entity ?: 1]."/trainingsession/".get_exdir(0, 0, 0, 1, $object);
}
...
$title = $langs->trans("TrainingSession").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/trainingsession/trainingsession_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	$modulepart = 'dolimeet';
	//todo:perms
	//$permissiontoadd = $user->rights->trainingsession->dolimeet->write;
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->trainingsession->dolimeet->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;

	//$relativepathwithnofile='dolimeet/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'trainingsession/'.dol_sanitizeFileName($object->ref).'/';
