...
/**
 *  \file       trainingsession_note.php
 *  \ingroup    trainingsession
 *  \brief      Tab for notes on TrainingSession
 */
...
$langs->loadLangs(array("dolimeet@dolimeet", "companies"));
...
$hookmanager->initHooks(array('trainingsessionnote', 'globalcard')); // Note that conf->hooks_modules contains array
...
$help_url = '';
llxHeader('', $langs->trans('TrainingSession'), $help_url);
...
if ($id > 0 || !empty($ref)) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/dolimeet/trainingsession_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
