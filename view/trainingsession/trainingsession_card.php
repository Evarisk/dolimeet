...
$refTrainingSessionMod = new $conf->global->DOLIMEET_TRAININGSESSION_ADDON();
...
$hookmanager->initHooks(array('trainingsessioncard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$upload_dir = $conf->dolimeet->multidir_output[$conf->entity ?: $conf->entity]."/trainingsession/".get_exdir(0, 0, 0, 1, $object);
// Load object
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/dolimeet/view/trainingsession/trainingsession_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/dolimeet/view/trainingsession/trainingsession_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
			}
		}
	}
	...
	if ($action == 'add' && $permissiontoadd) {
		...
		// Initialize object
		$now                   = dol_now();
		$object->ref           = $refTrainingSessionMod->getNextValue($object);
	...
	if ($action == 'confirm_delete' && GETPOST("confirm") == "yes")
	{
		...
		$urltogo = DOL_URL_ROOT . '/custom/dolimeet/view/trainingsession/trainingsession_list.php';
		header("Location: " . $urltogo);
		exit;
	}
...
//	include DOL_DOCUMENT_ROOT.'/core/actions_builddoc.inc.php';
		...
		if (is_numeric(GETPOST('model', 'alpha'))) {
			...
			if ($result <= 0) {
				...
				if ($conf->global->DOLIMEET_SHOW_DOCUMENTS_ON_PUBLIC_INTERFACE) {
					...
					$ecmfile->fetch(0, '', 'dolimeet/trainingsession/'.$object->ref.'/'.$filename, '', '', 'dolimeet_trainingsession', $id);
	...
	$triggersendname = 'DOLIMEET_TRAININGSESSION_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_TRAININGSESSION_TO';
	$trackid = 'trainingsession'.$object->id;
...
$title_edit   = $langs->trans("ModifyTrainingSession");
$help_url     = '';
...
if ($action == 'create') {
	...
	print '<input hidden class="flat" type="text" size="36" name="ref" id="ref" value="'.$refTrainingSessionMod->getNextValue($object).'">';
	print $refTrainingSessionMod->getNextValue($object);
	print '</td></tr>';
...
if (($id || $ref) && $action == 'edit' ||$action == 'confirm_setInProgress') {
	...
	print load_fiche_titre($langs->trans("EditTrainingSession"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'confirm_setInProgress' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("TrainingSession"), -1, "dolimeet@dolimeet");
	...
	if (($action == 'clone' && (empty($conf->use_javascript_ajax) || ! empty($conf->dol_use_jmobile)))		// Output when action = clone if jmobile or no js
		...
		$formquestionclone = array(
			'text' => $langs->trans("CloneTrainingSession", $object->ref),
		);

		$formconfirm .= $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('ToClone'), $langs->trans('ConfirmCloneTrainingSession', $object->ref), 'confirm_clone', $formquestionclone, 'yes', 'actionButtonClone', 350, 600);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/dolimeet/view/trainingsession/trainingsession_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	// Project
...
//			print '<a class="'. ($object->status == 0 ? 'butAction" id="actionButtonSign" href="' . DOL_URL_ROOT . '/custom/dolimeet/view/trainingsession/trainingsession_signature.php'.'?id='.$object->id.'&mode=init&token='.newToken().'"' : 'butActionRefused classfortooltip" title="'. $langs->trans('AlreadySigned').'"')  .' >' . $langs->trans("Sign") . '</a>' . "\n";
//			print '<span class="' . ($object->status == 1 ? 'butAction"  id="actionButtonLock"' : 'butActionRefused classfortooltip" title="' . dol_escape_htmltag($langs->trans("TrainingSessionMustBeSigned")) . '"') . '>' . $langs->trans("Lock") . '</span>';
