...
require_once __DIR__ . '/../lib/doliopi_function.lib.php';
...
$permissionToRead = $user->rights->doliopi->adminpage->read;
...
if ($action == 'set_session_trainer_responsible') {
    ...
    if ($responsibleId != getDolGlobalInt('DOLIOPI_SESSION_TRAINER_RESPONSIBLE')) {
        dolibarr_set_const($db, 'DOLIOPI_SESSION_TRAINER_RESPONSIBLE', $responsibleId, 'integer', 0, '', $conf->entity);
    }
...
if ($action == 'set_satisfaction_survey') {
    ...
    foreach ($satisfactionSurveys as $satisfactionSurvey) {
        ...
        $confName             = 'DOLIOPI_' . dol_strtoupper($satisfactionSurvey) . '_SATISFACTION_SURVEY_SHEET';
...
if ($action == 'update_formation_datas') {
    ...
    if ($categoryID != getDolGlobalInt('DOLIOPI_FORMATION_MAIN_CATEGORY')) {
        dolibarr_set_const($db, 'DOLIOPI_FORMATION_MAIN_CATEGORY', $categoryID, 'integer', 0, '', $conf->entity);
    }

    $categoryVariousID = GETPOST('formation_various_main_category', 'int');
    if ($categoryVariousID != getDolGlobalInt('DOLIOPI_FORMATION_VARIOUS_MAIN_CATEGORY')) {
        dolibarr_set_const($db, 'DOLIOPI_FORMATION_VARIOUS_MAIN_CATEGORY', $categoryVariousID, 'integer', 0, '', $conf->entity);
    }

    $trainingSessionTemplatesProject = GETPOST('training_session_templates_project', 'int');
    if ($trainingSessionTemplatesProject != getDolGlobalInt('DOLIOPI_TRAININGSESSION_TEMPLATES_PROJECT')) {
        dolibarr_set_const($db, 'DOLIOPI_TRAININGSESSION_TEMPLATES_PROJECT', $trainingSessionTemplatesProject, 'integer', 0, '', $conf->entity);
    }

    $createFromBackToPage = GETPOST('create_from_backtopage', 'int');
    if ($createFromBackToPage == 0) {
        $timePeriods = [
            'morning_start_hour'   => 'DOLIOPI_TRAININGSESSION_MORNING_START_HOUR',
            'morning_end_hour'     => 'DOLIOPI_TRAININGSESSION_MORNING_END_HOUR',
            'afternoon_start_hour' => 'DOLIOPI_TRAININGSESSION_AFTERNOON_START_HOUR',
            'afternoon_end_hour'   => 'DOLIOPI_TRAININGSESSION_AFTERNOON_END_HOUR'
        ];
        ...
        if ($trainingSessionLocation != getDolGlobalString('DOLIOPI_TRAININGSESSION_LOCATION')) {
            dolibarr_set_const($db, 'DOLIOPI_TRAININGSESSION_LOCATION', $trainingSessionLocation, 'chaine', 0, '', $conf->entity);
        }

        $trainingSessionAbsenceRate = GETPOST('training_session_absence_rate', 'int');
        if ($trainingSessionAbsenceRate != getDolGlobalInt('DOLIOPI_TRAININGSESSION_ABSENCE_RATE')) {
            dolibarr_set_const($db, 'DOLIOPI_TRAININGSESSION_ABSENCE_RATE', $trainingSessionAbsenceRate, 'integer', 0, '', $conf->entity);
        }
    }
...
print dol_get_fiche_head($head, 'settings', $title, -1, 'doliopi_color@doliopi');
...
print ajax_constantonoff('DOLIOPI_MEETING_MENU_ENABLED',  [], null, 0, 0, 1);
print '</td>';
print '</tr>';
...
print '<td class="center">';
print ajax_constantonoff('DOLIOPI_TRAININGSESSION_MENU_ENABLED',  [], null, 0, 0, 1);
print '</td>';
print '</tr>';
...
print '<td class="center">';
print ajax_constantonoff('DOLIOPI_AUDIT_MENU_ENABLED',  [], null, 0, 0, 1);
print '</td>';
print '</tr>';

print '</table>';

if (getDolGlobalInt('DOLIOPI_TRAININGSESSION_MENU_ENABLED')) {
    // Formation
    ...
    print $formOther->select_categories('product', getDolGlobalInt('DOLIOPI_FORMATION_MAIN_CATEGORY'), 'formation_main_category', 0, 1, 'minwidth300 maxwidth400 widthcentpercentminusx');
    ...
    print img_picto('', 'category', 'class="pictofixedwidth"');
    print $formOther->select_categories('product', getDolGlobalInt('DOLIOPI_FORMATION_VARIOUS_MAIN_CATEGORY'), 'formation_various_main_category', 0, 1, 'minwidth300 maxwidth400 widthcentpercentminusx');
    ...
    $formProjects->select_projects(-1, (GETPOSTISSET('training_session_templates_project') ? GETPOST('training_session_templates_project', 'int') : getDolGlobalInt('DOLIOPI_TRAININGSESSION_TEMPLATES_PROJECT')), 'training_session_templates_project', 0, 0, 0, 0, 0, 0, 0, '', 0, 0, 'minwidth300 maxwidth400 widthcentpercentminusxx');
    print ' <a href="' . DOL_URL_ROOT . '/projet/card.php?action=create&status=1&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?training_session_templates_project=&#95;&#95;ID&#95;&#95;&action=update_formation_datas&create_from_backtopage=1&token=' . newToken()) . '"><span class="fa fa-plus-circle valignmiddle" title="' . $langs->transnoentities('AddProject') . '"></span></a>';
    print '</td></tr>';

    // Training session durations
    print '<tr class="oddeven"><td>' . $langs->transnoentities('TrainingSessionDurations') . '</td><td>';
    print $langs->transnoentities('MorningStartHour') . '<input type="time" class="marginleftonly" name="morning_start_hour" value="' . getDolGlobalString('DOLIOPI_TRAININGSESSION_MORNING_START_HOUR') . '" /><br>';
    print $langs->transnoentities('MorningEndHour') . '<input type="time" class="marginleftonly" name="morning_end_hour" value="' . getDolGlobalString('DOLIOPI_TRAININGSESSION_MORNING_END_HOUR') . '" /><br>';
    print $langs->transnoentities('AfternoonStartHour') . '<input type="time" class="marginleftonly" name="afternoon_start_hour" value="' . getDolGlobalString('DOLIOPI_TRAININGSESSION_AFTERNOON_START_HOUR') . '" /><br>';
    print $langs->transnoentities('AfternoonEndHour') . '<input type="time" class="marginleftonly" name="afternoon_end_hour" value="' . getDolGlobalString('DOLIOPI_TRAININGSESSION_AFTERNOON_END_HOUR') . '" /><br>';
    print '</td></tr>';

    // Training session location
    print '<tr class="oddeven"><td>' . $langs->transnoentities('TrainingSessionLocation') . '</td><td>';
    print '<input type="radio" id="TrainingSessionLocationCompany" name="training_session_location" value="TrainingSessionLocationCompany"' . (getDolGlobalString('DOLIOPI_TRAININGSESSION_LOCATION') == 'TrainingSessionLocationCompany' ? 'checked' : '') . '/><label for="TrainingSessionLocationCompany">' . img_picto('', 'company', 'class="paddingright"') . $langs->transnoentities('TrainingSessionLocationCompany') . '</label><br>';
    print '<input type="radio" id="TrainingSessionLocationThirdParty" name="training_session_location" value="TrainingSessionLocationThirdParty"' . (getDolGlobalString('DOLIOPI_TRAININGSESSION_LOCATION') == 'TrainingSessionLocationThirdParty' ? 'checked' : '') . '/><label for="TrainingSessionLocationThirdParty">' . img_picto('', 'company', 'class="paddingright"') . $langs->transnoentities('TrainingSessionLocationThirdParty') . '</label><br>';
    print '<input type="radio" id="TrainingSessionLocationOther" name="training_session_location" value="TrainingSessionLocationOther"' . (getDolGlobalString('DOLIOPI_TRAININGSESSION_LOCATION') == 'TrainingSessionLocationOther' ? 'checked' : '') . '/><label for="TrainingSessionLocationOther">' . img_picto('', 'fontawesome_fa-font_fas', 'class="paddingright"') . $langs->transnoentities('TrainingSessionLocationOther') . '</label>';
    print '</td></tr>';
    ...
    print '<input type="number" name="training_session_absence_rate" min="0" max="100" value="' . getDolGlobalInt('DOLIOPI_TRAININGSESSION_ABSENCE_RATE') . '"/>';
    print '</td></tr>';
    ...
    print img_picto($langs->trans('User'), 'user', 'class="pictofixedwidth"') . $form->select_dolusers(getDolGlobalInt('DOLIOPI_SESSION_TRAINER_RESPONSIBLE'), 'session_trainer_responsible_id', 1, null, 0, '', '', '0', 0, 0, '', 0, '', 'minwidth400 maxwidth500');
    print '</td></tr>';
...
if (isModEnabled('digiquali') && version_compare(getDolGlobalString('DIGIQUALI_VERSION'), '1.11.0', '>=')) {
    ...
    foreach ($satisfactionSurveys as $satisfactionSurveyRole => $satisfactionSurvey) {
        ...
        print '</td>';
        print '<td class="minwidth400 maxwidth500">';
        $confName = 'DOLIOPI_' . dol_strtoupper($satisfactionSurveyRole) . '_SATISFACTION_SURVEY_SHEET';
