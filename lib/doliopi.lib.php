 ...
 */
function doliopi_admin_prepare_head(): array
{
    ...
    foreach ($sessionObjectTypes as $sessionObjectType => $picto) {
        if (!getDolGlobalInt('DOLIOPI_' . dol_strtoupper($sessionObjectType) . '_MENU_ENABLED')) {
            continue;
        }

        $imgPicto = img_picto('', 'fontawesome_' . $picto . '_fas', 'class="pictofixedwidth"');

        $head[$h][0] = dol_buildpath('/saturne/admin/object.php', 1) . '?module_name=Doliopi&object_type=' . $sessionObjectType;
    ...
    $head[$h][0] = dol_buildpath('saturne/admin/publicinterface.php', 1) . '?module_name=Doliopi';
    ...
    $head[$h][0] = dol_buildpath('doliopi/admin/financial_and_pedagogical_report.php', 1);
    ...
    $head[$h][0] = dol_buildpath('/saturne/admin/documents.php?module_name=Doliopi', 1);
    ...
    $head[$h][0] = dol_buildpath('/doliopi/admin/setup.php', 1);
    $head[$h][1] = $conf->browser->layout != 'phone' ? '<i class="fas fa-cog pictofixedwidth"></i>' . $langs->trans('ModuleSettings') : '<i class="fas fa-cog"></i>';
    $head[$h][2] = 'settings';
    $h++;

    $head[$h][0] = dol_buildpath('/saturne/admin/about.php?module_name=Doliopi', 1);
    $head[$h][1] = $conf->browser->layout != 'phone' ? '<i class="fab fa-readme pictofixedwidth"></i>' . $langs->trans('About') : '<i class="fab fa-readme"></i>';
    $head[$h][2] = 'about';
    $h++;

    complete_head_from_modules($conf, $langs, null, $head, $h, 'doliopi@doliopi');

    complete_head_from_modules($conf, $langs, null, $head, $h, 'doliopi@doliopi', 'remove');
