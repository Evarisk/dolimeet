...
/**
 * \file    class/actions_doliopi.class.php
 * \ingroup doliopi
 * \brief   Doliopi hook overload.
 */

/**
 * Class ActionsDoliopi.
 */
class ActionsDoliopi
{
    /**
    ...
    public function getElementProperties(array $parameters): int
    {
        if (preg_match('/elementproperties|category/', $parameters['context'])) {
            ...
            if (in_array($parameters['elementType'], $objectElements)) {
                $out = [
                    'module'        => 'doliopi',
                    'table_element' => 'doliopi_session',
                    'classpath'     => 'custom/doliopi/class',
                ];
                $this->results = $out;
            }
        }
    ...
    public function addHtmlHeader(array $parameters): int
    {
        ...
        if (preg_match('/thirdpartycontact/', $parameters['context'])) {
            $resourcesRequired = [
                'js'  => '/custom/doliopi/js/doliopi.min.js'
            ];
    ...
    public function formObjectOptions(array $parameters, $object, $action): int
    {
        ...
        foreach($objectsMetadata as $objectMetadata) {
            ...
            if (strpos($parameters['context'], $objectMetadata['hook_name_card']) !== false) {
                $pictoPath = dol_buildpath('/doliopi/img/doliopi_color.png', 1);
    ...
    public function printCommonFooter(array $parameters): int
    {
                ...
                foreach($linkedSessions as $linkedSession) {
                    switch ($linkedSession->type) {
                        ...
                        case 'audit':
                            ...
                            break;
                        default :
                            $linkedSession->picto = 'doliopi_color@doliopi';
                            break;
                    }
                        ...
                        if (in_array($contact['code'], $contactsCodeWanted)) {
                            if (isset($object->linkedObjectsIds['digiquali_survey']) && !empty($object->linkedObjectsIds['digiquali_survey'])) {
                                ...
                                foreach ($surveyIDs as $surveyID) {
                                    $confName = 'DOLIOPI_' . $contact['code'] . '_SATISFACTION_SURVEY_SHEET';
                                    ...
                                    if (getDolGlobalInt($confName) > 0) {
                                        ...
                                        $outputLine[$contact['rowid']] = '<td class="tdoverflowmax200">';
                                        $outputLine[$contact['rowid']] .= '<a href="' . dol_buildpath('/custom/doliopi/admin/setup.php', 1) . '">';
            ...
            if (!empty($contacts)) {
                ...
                foreach ($contacts as $contact) {
                    ...
                    if ($contact['mandatory_signature'] == 1) {
                        $outputLine[$contact['rowid']] .= '<a class="reposition" href="' . DOL_URL_ROOT . '/custom/doliopi/core/ajax/testajax.php?action=set&token=' . newToken() . '&rowid=' . ((int) $contact['rowid']) . '&value=0&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . $object->id) . '">' . img_picto($langs->trans('Enabled'), 'switch_on') . '</a>';
                    } else {
                        $outputLine[$contact['rowid']] .= '<a class="reposition" href="' . DOL_URL_ROOT . '/custom/doliopi/core/ajax/testajax.php?action=set&token=' . newToken() . '&rowid=' . ((int) $contact['rowid']) . '&value=1&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . $object->id) . '">' . img_picto($langs->trans('Disabled'), 'switch_off') . '</a>';
                    }
                    $outputLine[$contact['rowid']] .= '</td>';
                }
        ...
        if (preg_match('/categoryindex/', $parameters['context'])) {
            print '<script src="../custom/doliopi/js/doliopi.js"></script>';
        ...
        if (strpos($parameters['context'], 'contractcard') !== false) {
            ...
            if (isset($object->array_options['options_trainingsession_type']) && !empty($object->array_options['options_trainingsession_type'])) {
                ...
                $html = saturne_show_documents('doliopi:CompletioncertificateDocument', $dirFiles, $fileDir, $urlSource, $user->rights->contrat->creer, $user->rights->contrat->supprimer, '', 1, 0, 0, 0, 0, '', 0, $langs->defaultlang, '', $object, 0, 'remove_file', (($object->statut > Contrat::STATUS_DRAFT && getDolGlobalInt('DOLIOPI_SESSION_TRAINER_RESPONSIBLE') > 0) ? 1 : 0), $langs->trans('DefineSessionTrainerResponsible') . '<br>' . $langs->trans('ObjectMustBeValidatedToGenerate', ucfirst($langs->transnoentities(ucfirst($object->element))))); ?>
        ...
        if (strpos($parameters['context'], 'propalcard') !== false) {
            ...
            if ($trainingSessionType == 1) {
                require_once __DIR__ . '/../lib/doliopi_trainingsession.lib.php';
        ...
        if (strpos($parameters['context'], 'thirdpartycontact')) {
            ...
            ?>
            <script>
                window.doliopi.contactList.insertData(<?php echo json_encode($contactIds); ?>, '<?= $langs->trans('Formations') ?>');
            </script>
            <?php
        }
    ...
    public function addMoreActionsButtons(array $parameters, $object): int
    {
        ...
        if (preg_match('/categorycard/', $parameters['context'])) {
            ...
            $type      = GETPOST('type');
            if ($id > 0 && $elementId > 0 && ($user->rights->doliopi->$type->write)) {
                require_once __DIR__ . '/' . $type . '.class.php';
    ...
    public function doActions(array $parameters, $object, string $action): int
    {
        ...
        if ($parameters['currentcontext'] == 'doliopipublicinterfaceadmin') {
            if ($action == 'set_default_public_interface_user') {
                ...
                if ($newUserId > 0) {
                    ...
                    dolibarr_set_const($this->db, 'DOLIOPI_DEFAULT_PUBLIC_INTERFACE_USER', $newUserId, 'integer', 0, '', $conf->entity);
                    dolibarr_set_const($this->db, 'DOLIOPI_DEFAULT_PUBLIC_INTERFACE_USER_CREATED', $newUserId, 'integer', 0, '', $conf->entity);
                    setEventMessage($langs->trans('SavedConfig'));
                } else {
                    setEventMessages($userTmp->error, $userTmp->errors, 'errors');
                }

                header('Location: ' . $_SERVER['PHP_SELF'] . '?module_name=Doliopi');
                exit;
            }
        }
        ...
        if (strpos($parameters['context'], 'contractcard') !== false) {
            if ($action == 'builddoc' && strstr(GETPOST('model'), 'completioncertificatedocument_odt') || $action == 'builddoc' && strstr(GETPOST('model'), 'completioncertificatedocument')) {
                require_once __DIR__ . '/doliopidocuments/completioncertificatedocument.class.php';

                $document = new CompletioncertificateDocument($this->db);
                $document->element = 'trainingsessiondocument';

                $moduleNameLowerCase = 'doliopi';
                $permissiontoadd     = $user->rights->doliopi->trainingsession->write;
            ...
            if ($action == 'pdfGeneration') {
                ...
                $upload_dir          = $conf->doliopi->multidir_output[$conf->entity ?? 1];
            ...
            if ($action == 'send_email') {
                ...
                if (!empty($signatories['Attendant'])) {
                    ...
                    if (!empty($attendant->email)) {
                        ...
                        $substitutionarray['__DOLIOPI_SATIFACTION_SURVEY_LINK'] = dol_buildpath('custom/digiquali/public/public_answer.php?track_id=' . $survey->track_id . '&object_type=' . $survey->element . '&document_type=SurveyDocument&entity=' . $conf->entity, 3);
                        complete_substitutions_array($substitutionarray, $langs, $survey, $parameters);

                        $result  = $saturneMail->fetch(getDolGlobalInt('DOLIOPI_EMAIL_TEMPLATE_COMPLETION_CERTIFICATE_UNIQUE'));
    ...
    public function extendSheetLinkableObjectsList(array $linkableObjectTypes): int
    {
        ...
        $linkableObjectTypes['doliopi_trainsess'] = [
            'langs'          => 'Trainingsession',
            'langfile'       => 'doliopi@doliopi',
            'picto'          => $trainingSession->picto,
            'className'      => 'Trainingsession',
            'name_field'     => 'ref',
            'post_name'      => 'fk_trainingsession',
            'link_name'      => 'doliopi_trainsess',
            'tab_type'       => 'trainingsession',
            'hook_name_list' => 'trainingsessionlist',
            'hook_name_card' => 'trainingsessioncard',
            'create_url'     => 'custom/doliopi/view/session/session_card.php?action=create&object_type=trainingsession',
            'class_path'     => 'custom/doliopi/class/trainingsession.class.php'
        ];
    ...
    public function moreHtmlStatus(array $parameters, CommonObject $object): int
    {
        ...
        if (preg_match('/contractcard|contractcontactcard/', $parameters['context'])) {
            if (!empty($object->array_options['options_trainingsession_type'])) {
                ...
                if (getDolGlobalInt('DOLIOPI_CONTACT_PUBLIC_INTERFACE_ENABLED') > 0) {
                    $moreHtmlStatus .= '<a href="' . dol_buildpath('custom/doliopi/public/contact/add_contact.php', 3) . '?id=' . $object->id . '&token=' . currentToken() . '" target="_blank" class="butAction no-load">Interface Publique</a>';
                }
    ...
    public function completeTabsHead(array $parameters): int
    {
        ...
        if (strpos($parameters['context'], 'main') !== false) {
            ...
            switch ($parameters['object']->element ?? '') {
                case 'ticket':
                    return 0; // Tickets are not natively supported by doliopi_session
                case 'societe' :
                    $objectElement = 'soc';
                    break;
                case 'contract' :
                    ...
                    foreach ($parameters['head'] as $headKey => $tabsHead) {
                        if (is_array($tabsHead) && !empty($tabsHead)) {
                            if (isset($tabsHead[2]) && $tabsHead[2] === 'sessionList') {
                                ...
                                $picto     = img_picto('', $pictoPath, '', 1, 0, 0, '', 'pictoModule');
                                $parameters['head'][$headKey][1]  = $conf->browser->layout == 'classic' ? $picto . 'Doliopi' : $picto;
...
//            }
    ...
    public function saturneSetVarsFromFetchObj(array $parameters, object $object): int
    {
        ...
        if (preg_match('/meetinglist|trainingsessionlist|auditlist/', $parameters['context'])) {
            ...
            $signatory = new SaturneSignature($this->db, 'doliopi', $object->element);
        ...
        if (strpos($parameters['context'], 'sessionlist') !== false) {
            switch ($object->type) {
                ...
                case 'audit':
                    ...
                    break;
                default :
                    $object->picto  = 'doliopi_color@doliopi';
                    break;
            }
        }
    ...
    public function saturneIndex(array $parameters)
    {
        ...
        if (strpos($parameters['context'], 'doliopiindex') !== false) {
            $error          = 0;
            $formationConfs = [
                'integer' => [
                    'DOLIOPI_SERVICE_TRAINING_CONTRACT',
                    'DOLIOPI_SERVICE_WELCOME_BOOKLET',
                    'DOLIOPI_SERVICE_RULES_OF_PROCEDURE',
                    'DOLIOPI_FORMATION_MAIN_CATEGORY',
                    'DOLIOPI_FORMATION_VARIOUS_MAIN_CATEGORY',
                    'DOLIOPI_TRAININGSESSION_TEMPLATES_PROJECT',
                    'DOLIOPI_DEFAULT_PUBLIC_INTERFACE_USER'
                ],
                'chaine' => [
                    'DOLIOPI_TRAININGSESSION_MORNING_START_HOUR',
                    'DOLIOPI_TRAININGSESSION_MORNING_END_HOUR',
                    'DOLIOPI_TRAININGSESSION_AFTERNOON_START_HOUR',
                    'DOLIOPI_TRAININGSESSION_AFTERNOON_END_HOUR',
                    'DOLIOPI_TRAININGSESSION_LOCATION'
                ]
            ];
    ...
    {
        // Do something only for the current context.
        if ($parameters['currentcontext'] == 'doliopiadmindocuments') {
            $types = [
        ...
        if ($parameters['currentcontext'] == 'doliopiadmindocuments') {
            $constArray['doliopi'] = [
                'attendancesheetdocument' => [
                    ...
                    'code'        => 'DOLIOPI_ATTENDANCESHEETDOCUMENT_DISPLAY_ATTENDANCE_ABSENT_IN_SIGNATURE'
                ]
            ];
        ...
        if ($parameters['currentcontext'] == 'doliopipublicinterfaceadmin') {
            $constArray['doliopi'] = [
                'ContactPublicInterfaceEnabled' => [
                    ...
                    'code'        => 'DOLIOPI_CONTACT_PUBLIC_INTERFACE_ENABLED'
                ]
            ];
            $this->results = $constArray;
            return 1;
        ...
        if (strpos($parameters['context'], 'trainingsessioncard') !== false) {
            if (preg_match('/completioncertificate/', (!empty($parameters['models']) ? $parameters['models'][1] : $parameters['model']))) {
                ...
                if (is_array($signatoriesArray) && !empty($signatoriesArray)) {
                    ...
                    setEventMessages($langs->trans('FileGenerated') . ' - ' . '<a href=' . DOL_URL_ROOT . '/document.php?modulepart=doliopi&file=' . urlencode($document->element . '/' . $object->ref . '/' . $document->last_main_doc) . '&entity=' . $conf->entity . '"' . '>' . $document->last_main_doc, []);
                    $urlToRedirect = $_SERVER['REQUEST_URI'];
                ...
                if (is_array($sessions) && !empty($sessions)) {
                    ...
                    if (!empty($absentTrainees)) {
                        foreach ($absentTrainees as $absentId => $absentTrainee) {
                            if (($absentTrainee['nbAbsence'] / $nbSessions * 100) > getDolGlobalInt('DOLIOPI_TRAININGSESSION_ABSENCE_RATE')) {
